...
void getDirectional(float pl){
    if(pl == 0.5){ //bot
        ...
        if(dayTime < 1500){
            passCardinalLight =  mix(1.0f, 0.6f, dayTime/1500);
        }
        if(dayTime >= 1500 && dayTime < 22500){
            passCardinalLight = 0.6f;
        }
        if(dayTime >= 22500 && dayTime < 24000){
            passCardinalLight =  mix(0.6f, 1.0f, (dayTime-22500)/1500);
        }
    }else if(pl == 0.8){ //right
        passCardinalLight =  0.6f; //These aren't effected by either side
    }else if(pl == 0.9){ //left
        passCardinalLight =  0.6f; //These aren't effected by either side
    }else if(pl == 1.0){ //top
        if(dayTime < 6000){
            passCardinalLight =  mix(0.6f, 1.0f, dayTime/6000);
        }
        if(dayTime >= 6000 && dayTime < 12000){
            passCardinalLight =  mix(1.0f, 0.6f, (dayTime-6000)/6000);
        }
        if(dayTime >= 12000 && dayTime < 18000){
            passCardinalLight =  mix(0.6f, 1.0f, (dayTime-12000)/6000);
        }
        if(dayTime >= 18000 && dayTime < 24000){
            passCardinalLight =  mix(1.0f, 0.6f, (dayTime-18000)/6000);
        }
    }else if(pl == 0.7){ 
        ...
        if(dayTime >= 10500 && dayTime < 12000){
            passCardinalLight = mix(0.6f, 1.0f, (dayTime-10500)/1500);
        }
        if(dayTime >= 12000 && dayTime < 13500){
            passCardinalLight = mix(1.0f, 0.6f, (dayTime-12000)/1500);
        }
    }
}
